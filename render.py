...
# Create a function that sets all the necessary variables for the render
def create_render_variables(background_color, top_layer):
    background_color = tuple(map(int, background_color.split(",")))

    global BLUR, BACKGROUND_COLOR, TOP_LAYER
...
def generate(img, background_color, top_layer):
    create_render_variables(background_color, top_layer)
